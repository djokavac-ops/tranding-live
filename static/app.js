...
async function loadLiveIntelligence(){
  try{
    ...
    const cons=(s.consensus||[]).map(x=>`<div class="log"><b>${x.symbol}</b><br>${x.decision} · Consensus ${fmt(x.consensus_score)}</div>`).join("");
    $("liveIntelligence").innerHTML=`<div class="metric"><span>News Feed</span><strong>${s.news.enabled?"ON":"SCAFFOLD"}</strong></div><div class="metric"><span>Macro Calendar</span><strong>${s.macro_calendar.enabled?"ON":"SCAFFOLD"}</strong></div><div class="metric"><span>Self-learning runs</span><strong>${(s.self_learning_last||[]).length}</strong></div><div class="ai">${s.message}</div><h3>Consensus</h3>${cons}`;
  }catch(e){$("liveIntelligence").innerHTML="Live Intelligence trenutno nije dostupan."}
}
async function exportExcel(){ window.location.href="/api/export/excel"; }
async function runSelfLearning(){await api("/api/self-learning/run",{method:"POST"});$("status").textContent="Self-learning completed";await refresh();}
