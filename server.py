import os, time, json, sqlite3, math, threading
...
DB_PATH = DATA_DIR / "trading_ai_ultimate.db"
WATCHLIST_PATH = APP_DIR / "watchlist.json"

app = FastAPI(title="Trading AI Ultimate")
...
def fetch_chart(ticker, range_="1y", interval="1d"):
    ...
    r = requests.get(url, params={"range": range_, "interval": interval}, headers={"User-Agent": "Mozilla/5.0"}, timeout=15)
    r.raise_for_status()
...
def analyze(rows, meta):
    ...
    score=50; bull=[]; bear=[]
    if price>e20: score+=6; bull.append("price above EMA20")
    else: score-=6; bear.append("price below EMA20")
    if e8>e20>e50: score+=10; bull.append("bullish EMA stack")
    elif e8<e20<e50: score-=10; bear.append("bearish EMA stack")
    if e50>e200: score+=12; bull.append("primary trend bullish")
    else: score-=12; bear.append("primary trend bearish")
    score += 4 if e100>e200 else -4
    if mv>0: score+=6; bull.append("MACD positive")
    else: score-=6; bear.append("MACD negative")
    if 50<=rv<=66: score+=8; bull.append("RSI healthy")
    elif 66<rv<=72: score+=2; bull.append("RSI strong")
    elif rv>75: score-=9; bear.append("RSI overbought")
    elif rv<35: score-=9; bear.append("RSI weak/oversold")
    elif rv<45: score-=4
    if ch5>0 and ch20>0: score+=8; bull.append("5d and 20d momentum positive")
    elif ch5<0 and ch20<0: score-=8; bear.append("5d and 20d momentum negative")
    score += 5 if ch60>0 else -5
    if price>high20*0.995: score+=4; bull.append("near 20d breakout")
    if price<low20*1.005: score-=4; bear.append("near 20d breakdown")
    if price>high60*0.995: score+=3; bull.append("near 60d high")
    if price<low60*1.005: score-=3; bear.append("near 60d low")
    if volfactor>1.4: score+=3; bull.append("volume expansion")
    if bb>0.95: score-=4; bear.append("Bollinger high")
    if bb<0.05: score-=2; bear.append("Bollinger low")
    score += pscore
    ...
    reasons=(bull if side=="BUY" else bear)
    if side=="WAIT": reasons=(bull+bear)
    return {"symbol":None,"name":meta["name"],"ticker":meta["ticker"],"group":meta["group"],"price":round(price,4),
            ...
            "reason":"; ".join(reasons[:9]),"updated_at":now_iso()}

def log_decision(symbol,x):
    try:
        conn=db()
        conn.execute("INSERT INTO decisions(ts,symbol,name,decision,confidence,risk,price,reason) VALUES(?,?,?,?,?,?,?,?)",
                     (now_iso(),symbol,x.get("name",""),x.get("signal","ERR"),x.get("confidence",0),x.get("risk",""),x.get("price",0),x.get("reason","")))
        conn.commit(); conn.close()
...
def record_engine(status, opened, message):
    conn=db()
    conn.execute("INSERT INTO engine_runs(ts,status,opened_trade,message) VALUES(?,?,?,?)",(now_iso(),status,1 if opened else 0,message))
    conn.commit(); conn.close()
...
def run_cycle():
    with ENGINE_LOCK:
        ...
        if p>=settings["daily_target"]:
            msg="STOP: daily target reached"; record_engine("STOP",False,msg); return {"executed":False,"status":msg,"daily_pnl":p}
        if p<=-settings["daily_max_loss"]:
            msg="STOP: daily max loss reached"; record_engine("STOP",False,msg); return {"executed":False,"status":msg,"daily_pnl":p}
        if len(todays)>=int(settings["max_trades_per_day"]):
            msg="STOP: max trades reached"; record_engine("STOP",False,msg); return {"executed":False,"status":msg,"daily_pnl":p}
        if len(opened)>=int(settings["max_open_trades"]):
            msg="STOP: max open trades reached"; record_engine("STOP",False,msg); return {"executed":False,"status":msg,"daily_pnl":p}
        ...
        conn=db()
        conn.execute("""INSERT INTO paper_trades(day,opened_at,closed_at,symbol,name,group_name,side,entry,current_price,stop,target1,target2,qty,confidence,risk_reward,status,pnl,reason)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (today(),now_iso(),None,cand["symbol"],cand["name"],cand["group"],cand["side"],cand["entry"],cand["price"],cand["stop"],cand["target1"],cand["target2"],qty,cand["confidence"],cand["risk_reward"],"OPEN",0,cand["reason"]))
        conn.commit(); conn.close()
...
def api_paper():
    ...
    conn=db(); runs=conn.execute("SELECT * FROM engine_runs ORDER BY id DESC LIMIT 20").fetchall(); conn.close()
...
def api_alert(): return send_telegram("Trading AI Ultimate: Telegram connected.")
